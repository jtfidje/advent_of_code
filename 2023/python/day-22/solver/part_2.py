...
def solve(path: str):
    ...
    def check_above(brick: Brick, cache: dict[int, None]):
        cache[brick.label] = None

        if not brick.is_critical:
            return 0
        
        res = len(brick.bricks_above)
        for b_above in brick.bricks_above:
            if b_above.label in cache:
                continue

            res += check_above(b_above, cache)
    ...
    for brick in field.bricks:
        ...
        open_set: list[Brick] = [*brick.bricks_above]
        while open_set:
            _brick = open_set.pop(0)
            if all([b.has_fallen for b in _brick.bricks_below]):
                result += 1
                _brick.has_fallen = True
                open_set.extend([b for b in _brick.bricks_above if b not in open_set])
